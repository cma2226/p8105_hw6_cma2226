...
Backward selection is a method of model variable selection in which the least significant predictors are sequentially removed from the model until only significant predictors remain. This is the approach I adopted in developing my linear regression model. 
...
The plot illustrates the majority of values as being centered around a residual value of 0 and predicted value between 2500 and 3500. As the predicted weight values get very small, however, we see higher residual values, which suggest that this model performs well for average and above average weight babies, but not as well for lower weight babies. 
...
Now I will generate a plot of model residuals against fitted values. 
...
As we can see from this plot, the plot made through backward stepwise selection has the lowest RMSE. Thus, this model performs the best in predicting baby weight. 

### Problem 3

For this problem, I will use a dataset documenting weather observed in Central Park. We are interested in predicting the daily reported temperature maximum using the reported temperature minimum as a predictor.
...
I will use 5000 bootstrap samples to generate estimates for $\hat r^2$ and $log(\beta_0*\beta_1)$ for each bootstrap. 
...
The mean $\hat r^2$ over all bootstrap samples is calculated as follows:
...
Now I will generate the $log(\beta_0*\beta_1)$ variable using estimates for $\beta_0$ and $\beta_1$ for each bootstrap.
...
DESCRIBE PLOT IN WORDS: Normal, r^2 is maybe skewed left? 
...
DESCRIBE PLOT IN WORDS

Using the 5000 bootstrap estimates, I will now identify the 2.5% and 97.5% quantiles to provide a 95% confidence interval for $\hat r^2$2 and $log(\beta_0*\beta_1)$1).
